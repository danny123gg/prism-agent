...
后来在 Vida 的引导下，我接触到了侯世达（Douglas Hofstadter）关于"递归"与"自指"的思想。这些概念听起来很抽象，但它们给了我一个新的视角：与其在外面猜测黑盒里发生了什么，不如自己动手把它拆开看看。

于是我开始复刻。

Vida 带我完成了 14 个渐进阶段的练习，每个阶段聚焦一个核心概念。跑通之后，我又用 Claude Agent SDK 重新实现了一遍——也就是这个项目。

技术架构不同，但学习方法是一脉相承的：不追求全面覆盖，而是找一条最短的路径。从"能跑起来"开始，每次只解决一个问题。

六个核心阶段，两个扩展主题。每一步都建立在前一步的基础上。

这不是官方教程，只是一个人的学习笔记。如果有错误或更好的理解方式，欢迎指正。

> 如果你只是想快速上手，可以直接部署后去前台体验，尤其是看 Playground——那里能直观地看到 Agent 的运作过程。如果你想了解设计理念，可以继续往下读。
